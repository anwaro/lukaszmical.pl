...
import {BiggestValueResolvedResolver} from './resolver-biggest-value-resolved';

describe('BiggestGroupCheckedResolver', () => {
    const resolver = new BiggestValueResolvedResolver();
    ...
    it('should exclude cells near group equal to biggest value, and second biggest', () => {
        const cells = factory
            .init()
            .addCells(CellStatus.unknown)
    ...
    it('should exclude group border with size 2 all group with biggest value are resolved', () => {
        const cells = factory
            .init()
            .fromPattern('🟦🟦🟦🟦❌❔❔❔❔🟦🟦❔❔❔❔❌❌❌❌❌')
            .getCells();
        const group = factory.getGroup([4, 2, 2, 2]);
        const result = resolver.run(group, cells);

        expect(result.getIncludedCellsId()).toEqual([]);
        expect(result.getExcludedCellsId()).toEqual(['A9', 'A12']);
    });
});
