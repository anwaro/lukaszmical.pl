...
export class ImageDataHelper {
    ...
    static isSimilarColor(color1: Color, color2: Color) {
        return (
            Math.abs(color1[0] - color2[0]) < 3 &&
            Math.abs(color1[1] - color2[1]) < 3 &&
            Math.abs(color1[2] - color2[2]) < 3 &&
            Math.abs(color1[3] - color2[3]) < 3
        );
    }
