...
var urlPr = '/projects/flappy/image/';
var game = {
    user: {
        best: 0,
        score: 0,
    ...
    column: {
        img_top: new Image(),
        img_bot: new Image(),
        src_top: urlPr + 'col_top.png',
        src_bot: urlPr + 'col_bott.png',
    },
    canv: '',
    ctx: '',
    loop: '',
...
game.setColumn = function (b, i) {
    var z = i === 0 ? 2 : i - 1,
        y =
            Math.floor(
                Math.random() * (game.set.y - game.set.ground - game.set.win - 200),
            ) + 100;
...
game.scoreUp = function (c) {
    if (
        Math.abs(game.bird.x - (c.x + c.w / 2)) < game.set.speed + 1 &&
        game.user.Up
    ) {
        game.user.score++;
        game.user.Up = false;
