...
ENGINE.cal = function () {
    ...
    SET.temp =
        Math.pow(
            (SET.sun * (1 - SET.wAlbedo) * (1 + SET.tauVis)) /
                (SET.boltz * (1 + SET.tauIR)),
            1 / 4,
        ) - 273;
    document.getElementById('tmperature').innerHTML = SET.temp.toFixed(2) + '&deg;C';
    document.getElementById('tmperature').style.color = ENGINE.setTempColor(
        SET.temp,
    );
};
