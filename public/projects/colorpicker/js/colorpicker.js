colorpicker = function (elementId, callback) {
    ...
    var canvasArea = createElement('canvas', {
        className: 'colorPickerArea',
        height: 200,
        width: 200,
    });
    var canvasBar = createElement('canvas', {
        className: 'colorPickerBar',
        height: 155,
        width: 40,
    });
    var canvasColor = createElement('canvas', {
    ...
    colorPicker.addEventListener('focusout', function () {
        alert('ll');
        colorPicker.style.display = 'none';
    });

    canvasArea.addEventListener('mousedown', function () {
        areaMouseDown = true;
    });
    canvasBar.addEventListener('mousedown', function () {
        barMouseDown = true;
    });
    ...
    canvasBar.addEventListener('mouseup', function () {
        barMouseDown = false;
    });

    canvasArea.addEventListener('mousemove', function (event) {
        areaMouseDown && areaClick(event);
    });
    canvasBar.addEventListener('mousemove', function (event) {
        barMouseDown && barClick(event);
    });

    canvasArea.addEventListener('mouseleave', function () {
        areaMouseDown = false;
    });
    canvasBar.addEventListener('mouseleave', function () {
        barMouseDown = false;
    });
