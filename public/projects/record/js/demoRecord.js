...
function drawP() {
    ...
    for (var i = 0; i < num; i++) {
        ...
        if (
            planets[i].poss.x - planets[i].r + planets[i].v.x <= 0 ||
            planets[i].poss.x + planets[i].r + planets[i].v.x >= canvasWidth
        )
            planets[i].v.x *= -f;
        if (
            planets[i].poss.y - planets[i].r + planets[i].v.y <= 0 ||
            planets[i].poss.y + planets[i].r + planets[i].v.y >= canvasHeight
        )
            planets[i].v.y *= -f;
...
function getForce(p) {
    ...
    var force = (-0.15 / r) * r;
    return {
        x: (force * (p.poss.x - mouseX)) / r,
        y: (force * (p.poss.y - mouseY)) / r,
    };
}
