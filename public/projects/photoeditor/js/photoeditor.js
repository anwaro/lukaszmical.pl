...
$photo = new (function () {
    ...
    function preapareData() {
        if (combo) {
            photoData = ctxInput.getImageData(
                0,
                0,
                canvasInput.width,
                canvasInput.height,
            ...
            photoData = ctxOrginal.getImageData(
                0,
                0,
                canvasOrginal.width,
                canvasOrginal.height,
            );
        }
    ...
    method['szary'] = function () {
        var average,
            dark = rangeValue * 2.55;
        for (var i = 0; i < photoData.height; i++) {
            for (var j = 0; j < photoData.width; j++) {
                average =
                    (outputPhotoData[i][j].r +
                        outputPhotoData[i][j].g +
                        outputPhotoData[i][j].b) /
                        3 -
                    dark;
    ...
    method['kontur1'] = function () {
        ...
        for (var i = 0; i < photoData.height - 1; i++) {
            for (var j = 0; j < photoData.width - 1; j++) {
                outputPhotoData[i][j].r =
                    Math.abs(copy[i][j].r - copy[i + 1][j + 1].r) +
                    Math.abs(copy[i + 1][j].r - copy[i][j + 1].r);
                outputPhotoData[i][j].g =
                    Math.abs(copy[i][j].g - copy[i + 1][j + 1].g) +
                    Math.abs(copy[i + 1][j].g - copy[i][j + 1].g);
                outputPhotoData[i][j].b =
                    Math.abs(copy[i][j].b - copy[i + 1][j + 1].b) +
                    Math.abs(copy[i + 1][j].b - copy[i][j + 1].b);
            }
        }
    };
    ...
    method['circle'] = function () {
        ...
        ctxInput.putImageData(
            ctxOutput.getImageData(0, 0, canvasInput.width, canvasInput.height),
            0,
            0,
        );
    };
    ...
    method['bit'] = function () {
        ...
        for (var i = 0; i < photoData.height; i++) {
            for (var j = 0; j < photoData.width; j++) {
                ...
                outputPhotoData[i][j].r =
                    Math.round(color.r / rangeValue) * rangeValue;
                outputPhotoData[i][j].g =
                    Math.round(color.g / rangeValue) * rangeValue;
                outputPhotoData[i][j].b =
                    Math.round(color.b / rangeValue) * rangeValue;
            }
        }
    };
     ...
     */
    function mask(mask) {
        if (!(Math.sqrt(mask.length) % 2)) return false;

        var color,
            size = Math.sqrt(mask.length),
            margin = Math.floor(size / 2),
            sum = Math.abs(
                mask.reduce(function (pv, cv) {
                    return pv + cv;
                }, 0),
            ),
            copy = JSON.parse(JSON.stringify(outputPhotoData));
