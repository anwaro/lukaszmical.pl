$$div = new (function () {
    ...
    this.init = function () {
        ...
        for (var i = 0, t = 0, l = 0; i < box.length; i++) {
            ...
            div.css({
                ...
                left: l * divSize + 'px',
                background:
                    'url(' + url + ') -' + l * divSize + 'px -' + t * divSize + 'px',
            });
    ...
    function animation() {
        ...
        for (var i = 0; i < divs.length; i++) {
            var div = divs[i];
            if (
                Math.abs(div.direction.x - div.current.x) >
                2 * Math.abs(div.step.x)
            ) {
                moveStop = false;
            ...
            if (
                Math.abs(div.direction.y - div.current.y) >
                2 * Math.abs(div.step.y)
            ) {
                moveStop = false;
