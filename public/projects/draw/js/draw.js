...
$$Draw = new (function () {
    ...
    function drawElement(elem, curr, _ctx) {
        var c = curr
            ? {x: mouseX, y: mouseY}
            : {x: elem.point[1].x, y: elem.point[1].y};
        ...
        if (elem.type === 'line' && elem.point.length > 1) {
            if (
                elem.point[0].x !== elem.point[1].x ||
                elem.point[0].y !== elem.point[1].y
            ) {
            ...
            if (shiftDown && curr) {
                if (
                    Math.abs(elem.point[0].x - c.x) < Math.abs(elem.point[0].y - c.y)
                ) {
    ...
    function drawEnd() {
        if (isDraw) {
            if (currentDraw.type === 'straightLine' && shiftDown) {
                if (
                    Math.abs(currentDraw.point[0].x - mouseX) <
                    Math.abs(currentDraw.point[0].y - mouseY)
                ) {
                    currentDraw.point.push(
                        new Point(currentDraw.point[0].x, mouseY),
                    );
                } else {
                    currentDraw.point.push(
                        new Point(mouseX, currentDraw.point[0].y),
                    );
                }
            } else {
                currentDraw.point.push(new Point(mouseX, mouseY));
    ...
    function touchMove(a) {
        if (a.touches.length === 1) {
            ...
            mouseX = a.touches[0].pageX - (window.innerWidth - canvasWidth) * 0.5;
            mouseY =
                a.touches[0].pageY - (window.innerHeight - canvasHeight) * 0.5 - 20;
        }
    }
