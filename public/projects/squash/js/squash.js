...
function Balls(color, rad) {
    ...
    (this.color = color),
        (this.dX =
            Math.pow(-1, Math.ceil(Math.random() * 10) % 10) *
            (Math.random() * 3 + 2));
    this.dY = -9;
    this.radius = rad;
}
...
for (var i = 0; i < Game.NumTrance; i++) {
    Trance[i] = new Balls(
        'rgb(255,' + (80 + i) + ',0)',
        Ball.radius - ((Ball.radius - 1) / Game.NumTrance) * (i + 1),
    );
}
...
function MakeGradient() {
    ...
    BarGradient = ctx.createLinearGradient(
        0,
        Game.boardYwithMenu - 30,
        0,
        Game.boardYwithMenu,
    );
    ...
    BlockGradient[1] = ctx.createLinearGradient(
        0,
        0,
        0,
        Game.BlockHeight * Game.NumY + Game.MenuY,
    );
    BlockGradient[1].addColorStop(0, 'yellow');
    BlockGradient[1].addColorStop(0.4, 'orange');
    BlockGradient[1].addColorStop(1, 'yellow');

    BlockGradient[2] = ctx.createLinearGradient(
        0,
        0,
        0,
        Game.BlockHeight * Game.NumY + Game.MenuY,
    ...
    BlockGradient[3] = ctx.createLinearGradient(
        0,
        0,
        0,
        Game.BlockHeight * Game.NumY + Game.MenuY,
...
function MoveBar() {
    ...
    else if (Mouse.MouseX > Game.boardX - Bar.width / 2)
        Bar.posX = Game.boardX - Bar.width / 2;
...
function MoveBall() {
    ...
    if (Ball.posY + Ball.radius >= Game.boardYwithMenu - 60) {
        if (Ball.posX >= x1 - Bar.height / 2 && Ball.posX <= x2 + Bar.height / 2) {
            if (
                Ball.posX * a + b - Bar.height / 2 <=
                    Ball.posY + Ball.dY + Ball.radius &&
                Ball.dY > 0
            ) {
                if (Alfa > 0) {
                    //  \.
    ...
    if (
        Ball.posX + Ball.dX > Game.boardX - Ball.radius ||
        Ball.posX + Ball.dX < Ball.radius
    )
        Ball.dX = -Ball.dX;
    if (
        Ball.posY + Ball.dY > Game.boardYwithMenu - Ball.radius ||
        Ball.posY + Ball.dY < Game.MenuY + Ball.radius
    )
        Ball.dY = -Ball.dY;
}
...
function ResetBallPoss() {
    Ball.dX =
        Math.pow(-1, Math.ceil(Math.random() * 10) % 10) * (Math.random() * 3 + 2);
    Ball.dY = -9;
        ...
        for (let j = 0; j < Game.NumY; j++) {
            if (Area[i][j].display > 0) {
                if (Ball.dY > 0 && Ball.dX > 0)
                    if (
                        Area[i][j].posX <= Ball.posX + Ball.radius &&
                        Ball.posX + Ball.radius <=
                            Area[i][j].posX + Game.BlockWidth &&
                        Area[i][j].posY <= Ball.posY + Ball.radius &&
                        Ball.posY + Ball.radius <= Area[i][j].posY + Game.BlockHeight
                    ) {
                        Area[i][j].display--;
                        Game.Score++;
                        if (
                            Area[i][j].posY <= Ball.posY &&
                            Ball.posY <= Area[i][j].posY + Game.BlockHeight
                        )
                            Ball.dX = -Ball.dX;
...
function WhereMouse(canvas, evt) {
    ...
    if (Math.abs((Mouse.dM + Mouse.dM2) / 2) > 30)
        Mouse.dM =
            (Math.abs((Mouse.dM + Mouse.dM2) / 2) / ((Mouse.dM + Mouse.dM2) / 2)) *
            30;
