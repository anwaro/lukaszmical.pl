(function () {
    ...
    var val =
        'var data = [\n\t["Pon",2],\n\t["Wt",5],\n\t["Śro", 1],\n\t["Czw", 7],\n\t["Pią", 4],\n\t["Sob", 3],\n\t["Nied", 3]\n];\n\n';
    val +=
        'document.getElementById("graph1").graph(data,{\n\t"canvasHeight":300\n\t"canvasWidth":500\n});';
    ...
    codeMirrorJsOwn = CodeMirror($$('#js-graph-own'), {
        lineNumbers: true,
        readOnly: true,
    });
    codeMirrorHtmlOwn = CodeMirror($$('#html-graph-own'), {
        lineNumbers: true,
        readOnly: true,
        mode: 'text/html',
    });
    val = '<html>\n\t<head>\n\t\t.\n\t\t.\n\t\t';
    val +=
        '<script src="https://lukaszmical.pl/projects/graph/js/graph.js"></script> \n\t\t.\n\t\t.\n\t';
    val += '</head>\n\t<body>\n\t\t.\n\t\t.\n\t\t';
    val +=
...
function isBool(val) {
    ...
    else if (/^[\-\+]?[0-9]*(\.[0-9]+)?$/.test(val)) return parseFloat(val);
