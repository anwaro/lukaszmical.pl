$math = new (function () {
    ...
    function scrollCanv(e) {
        if (!proces && scrollActive) {
            e.preventDefault();
            minX = parseFloat(
                document.querySelector('#xmin').value.replace(',', '.'),
            );
            maxX = parseFloat(
                document.querySelector('#xmax').value.replace(',', '.'),
            );
            document.querySelector('#xmin').value = fixNumber(
                e.deltaY > 0
                    ? minX + stepX * 0.6 * (mouseX / x)
                    : minX - stepX * 0.6 * (mouseX / x),
                stepX / 10,
            );
            document.querySelector('#xmax').value = fixNumber(
                e.deltaY > 0
                    ? maxX - stepX * 0.6 * (1 - mouseX / x)
                    : maxX + stepX * 0.6 * (1 - mouseX / x),
                stepX / 10,
            );
            draw();
    ...
    function drawGraph() {
        ...
        if (lineActive) {
            ctx.moveTo(
                margin + (data[0][0].x - minX) * dX,
                margin + (maxY - data[0][0].y) * dY,
            );
            for (var i in data) {
                for (var j in data[i]) {
                    ctx.lineTo(
                        margin + (data[i][j].x - minX) * dX,
                        margin + (maxY - data[i][j].y) * dY,
                    );
                }
            }
            ctx.stroke();
