const data = {
    animal: {
        url: 'animal',
        name: 'Animal',
        description: 'Animal animation',
        css: ['style.css'],
        js: ['animal.js'],
        date: '2018-12-27',
        blank: true,
    },
    animate: {
        url: 'animate',
        name: 'Animacja',
        description: 'Prosta animacja',
        css: ['animate.css'],
        js: ['myQuery.js', 'animate.js'],
        date: '2015-05-22',
        blank: true,
    },
    audio: {
        url: 'audio',
        name: 'Audio Visualizer 3D',
        description: '',
        css: ['audio.css'],
        js: ['myQuery.js', 'glMatrix-0.9.5.min.js', 'audio.js'],
        date: '2016-09-23',
        blank: true,
    },
    billiards: {
        url: 'billiards',
        name: 'Bilard',
        description: 'Bilard',
        css: [],
        js: ['billiards.js'],
        date: '2015-05-22',
        blank: true,
    },
    canvas3d: {
        url: 'canvas3d',
        name: 'Canvas 3D',
        description: 'Biblioteka 3d',
        css: ['canvas3d.css'],
        js: ['object3d.js', 'canvas3d.js', 'demo3d.js'],
        date: '2016-09-27',
        blank: true,
    },
    car3d: {
        url: 'car3d',
        name: 'Car 3D model',
        description: 'Model samochodu ',
        css: ['car3D.css'],
        js: ['three.min.js', 'OBJLoader.js', 'car3D.js'],
        date: '2016-02-12',
        blank: true,
    },
    clock: {
        url: 'clock',
        name: 'Clock',
        description: 'Zegarek analogowy',
        css: ['clock.css'],
        js: ['clock.js'],
        date: '2015-11-13',
        blank: true,
    },
    colorpicker: {
        url: 'colorpicker',
        name: 'Kolor picker',
        description: '',
        css: [],
        js: ['colorpicker.js', 'demo.js'],
        date: '2016-03-14',
        blank: true,
    },
    draw: {
        url: 'draw',
        name: 'Paint',
        description: 'Internetowy paint',
        css: ['draw.css'],
        js: ['myQuery.js', 'draw.js'],
        date: '2015-05-28',
        blank: true,
    },
    drop: {
        url: 'drop',
        name: 'Drops',
        description: 'Ruch bąbelków',
        css: ['drop.css'],
        js: ['drop.js'],
        date: '2017-07-11',
        blank: true,
    },
    flappy: {
        url: 'flappy',
        name: 'Flappy Birds',
        description: 'Popularna gra flappy birds.',
        css: ['flappy.css'],
        js: ['myQuery.js', 'flappy-bird.js'],
        date: '2015-05-20',
        blank: true,
    },
    gauss: {
        url: 'gauss',
        name: 'Gauss',
        description: 'Rozkład liczb pseudolosowych',
        css: ['gauss.css'],
        js: ['gauss.js'],
        date: '2016-02-23',
        blank: true,
    },
    graf: {
        url: 'graf',
        name: 'Graf',
        description: 'Grafik',
        css: ['graf.css'],
        js: ['graf.js'],
        date: '2016-06-02',
        blank: true,
    },
    graph: {
        url: 'graph',
        name: 'Graph',
        description: 'Aplikacja rysująca wykresy',
        css: ['codemirror.css', 'graph.css'],
        js: ['myQuery.js', 'codemirror.js', 'graph.js', 'demoGraph.js'],
        date: '2015-11-07',
        blank: true,
    },
    grid: {
        url: 'grid',
        name: 'Particle',
        description: 'Cząstki oddziałujące z kursorem',
        css: ['grid.css'],
        js: ['grid.js'],
        date: '2017-06-28',
        blank: true,
    },
    hangman: {
        url: 'hangman',
        name: 'Wisielec',
        description: 'Gra wisielec',
        css: ['hangman.css'],
        js: ['myQuery.js', 'hangman.js'],
        date: '2015-11-18',
        blank: true,
    },
    jigsaw: {
        url: 'jigsaw',
        name: 'Układanka',
        description: 'Układanka z zdjęcia',
        css: ['theme.css', 'jigsaw.css'],
        js: ['myQuery.js', 'jigsaw.js'],
        date: '2015-11-09',
        blank: true,
    },
    jump: {
        url: 'jump',
        name: 'Jump',
        description: '-',
        css: ['jump.css'],
        js: ['jump.js'],
        date: '2016-02-20',
        blank: true,
    },
    life: {
        url: 'life',
        name: 'Gra w życie',
        description: '',
        css: ['theme.css', 'life.css'],
        js: ['myQuery.js', 'life.js'],
        date: '2016-10-02',
        blank: true,
    },
    math: {
        url: 'math',
        name: 'Wykresy ',
        description: 'Aplikacja do rysowania wykresu funkcji.',
        css: ['theme.css', 'math.css', 'mathcode.css'],
        js: ['mathcode.js', 'math.new.js'],
        date: '2015-05-06',
        blank: true,
    },
    mathcode: {
        url: 'mathcode',
        name: 'Math Code',
        description: 'Kod funckcji',
        css: ['mathcode.css'],
        js: ['mathcode.js', 'demo.js'],
        date: '2016-03-10',
        blank: true,
    },
    md5: {
        url: 'md5',
        name: 'Md5 Generator',
        description:
            'This online tool allows you to generate the MD5 hash of any string.',
        css: ['md5.css'],
        js: ['md5.js', 'index.js'],
        date: '2018-07-01',
        blank: true,
    },
    mp3player: {
        url: 'mp3player',
        name: 'Mp3 Player',
        description: 'Odtwarzacz muzyki',
        css: ['mp3player.css'],
        js: ['mp3player.js'],
        date: '2015-11-19',
        blank: true,
    },
    'my-query': {
        url: 'my-query',
        name: 'myQuery',
        description: 'Mała biblioteka js',
        css: ['myQuery.css'],
        js: [],
        date: '2016-09-19',
        blank: true,
    },
    photo: {
        url: 'photo',
        name: 'Pixel photo',
        description: 'Dodaje efekt łączenia kolorów w większe piksele.',
        css: ['photo.css'],
        js: ['photo.js'],
        date: '2015-05-06',
        blank: true,
    },
    photoeditor: {
        url: 'photoeditor',
        name: 'Edytor zdjęć',
        description: 'Dodaje filtry do zdjęcia.',
        css: ['photoeditor.css'],
        js: [
            'myQuery.js',
            'colorpicker.js',
            'photo.filter.mask.js',
            'photoeditor.js',
        ],
        date: '2016-03-10',
        blank: true,
    },
    photofilter: {
        url: 'photofilter',
        name: 'Photo filter',
        description: 'Dodaje filtry do zdjęcia.',
        css: ['photofilter.css'],
        js: [
            'myQuery.js',
            'colorPicker.js',
            'photo.filter.mask.js',
            'photo.filter.js',
        ],
        date: '2015-05-04',
        blank: true,
    },
    plot: {
        url: 'plot',
        name: 'Wykresy ',
        description: 'Aplikacja do rysowania wykresu funkcji.',
        css: ['theme.css', 'plot.css'],
        js: ['math.js'],
        date: '2015-05-27',
        blank: true,
    },
    puzzles: {
        url: 'puzzles',
        name: 'Puzzle',
        description: 'Układaj puzzle z własnych zdjęć',
        css: ['theme.css', 'puzzles.css'],
        js: ['myQuery.js', 'puzzles.js'],
        date: '2015-05-14',
        blank: true,
    },
    record: {
        url: 'record',
        name: 'Record',
        description: 'Nagrywanie filmu z elementu canvas',
        css: ['record.css'],
        js: ['record.js', 'demoRecord.js'],
        date: '2015-11-12',
        blank: true,
    },
    'rock-paper-scissors': {
        url: 'rock-paper-scissors',
        name: 'Kamień papier nożyce',
        description: 'Kamień papier nożyce',
        css: ['rock-paper-scissors.css'],
        js: ['rock-paper-scissors.js'],
        date: '2017-07-20',
        blank: true,
    },
    rubikscube: {
        url: 'rubikscube',
        name: 'Kostka rubika',
        description: '',
        css: ['rubikscube.css'],
        js: ['rubikscube.js'],
        date: '2016-03-16',
        blank: true,
    },
    run: {
        url: 'run',
        name: 'Run calculator',
        description: 'Calculate speed, pace, distance and time',
        css: ['bootstrap.min.css', 'run.css'],
        js: ['run.js'],
        date: '2017-06-07',
        blank: true,
    },
    runner: {
        url: 'runner',
        name: 'Runner',
        description: 'Runner animation',
        css: ['runner.css'],
        js: [],
        date: '2018-12-27',
        blank: true,
    },
    squash: {
        url: 'squash',
        name: 'Squash',
        description: 'Squash',
        css: ['squash.css'],
        js: ['squash.js'],
        date: '2015-05-21',
        blank: true,
    },
    stayalive: {
        url: 'stayalive',
        name: 'Stay Alive',
        description: 'Ruszaj piłką i unikaj min',
        css: ['stayalive.css'],
        js: ['myQuery.js', 'buzz.min.js', 'CanvasCurve.js', 'StayAlive.js'],
        date: '2014-11-12',
        blank: true,
    },
    sync: {
        url: 'sync',
        name: 'Sync',
        description: '-',
        css: ['sync.css'],
        js: ['sync.js'],
        date: '2016-06-29',
        blank: true,
    },
    temp: {
        url: 'temp',
        name: 'Tenperatura',
        description: 'Temperaturę na ziemi w zależności od parametrów.',
        css: ['temp.css'],
        js: ['setting.js', 'engine.js', 'draw.js', 'event.js', 'main.js'],
        date: '2014-06-12',
        blank: true,
    },
    test: {
        url: 'test',
        name: 'Test',
        description: 'Test',
        css: ['test.css'],
        js: ['myQuery.js', 'test.js'],
        date: '2015-05-20',
        blank: true,
    },
    tetris: {
        url: 'tetris',
        name: 'Tetris',
        description: 'Tetris',
        css: ['theme.css', 'style.css'],
        js: ['tetris.js'],
        date: '2015-05-21',
        blank: true,
    },
    text: {
        url: 'text',
        name: 'Text Statistics',
        description: 'Text Statistics',
        css: ['text.css'],
        js: ['text.js'],
        date: '2017-06-16',
        blank: true,
    },
    visualizer: {
        url: 'visualizer',
        name: 'Visualiser 3D',
        description: 'Audio visualizer 3D',
        css: ['visualizer.css'],
        js: ['myQuery.js', 'object3d.js', 'canvas3d.js', 'visualizer.js'],
        date: '2016-09-20',
        blank: true,
    },
};

export type Project = {
    url: string;
    name: string;
    description: string;
    css: string[];
    js: string[];
    date: string;
    blank: boolean;
};

export type Projects = {
    [key: string]: Project;
};

export const projects: Projects = data;
